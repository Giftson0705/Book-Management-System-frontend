...
export function setupMyBooks(container) {
  const token = localStorage.getItem("token");
  if (!token) {
  ...
  const myBooksList = container.querySelector("#mybooks-list");
  const logoutBtn = container.querySelector("#logout-btn-mybooks");
