...
// const ADMIN_BOOKS_API = `${API_BASE}/admin/books`;
// const token = localStorage.getItem("token");
...
export function setupAdminBooks(container) {
  const token = localStorage.getItem("token");
  if (!token) {
  ...
  const booksList = container.querySelector("#admin-books-list");
  const addBookForm = container.querySelector("#add-book-form");
  const logoutBtn = container.querySelector("#logout-btn-adminbooks");
  ...
  function renderBooks(books) {
    ...
    books.forEach((book) => {
      ...
      editBtn.onclick = () =>
        editBook(book._id, book.title, book.author, book.genre);
  ...
  addBookForm?.addEventListener("submit", async (e) => {
    ...
    const genre = addBookForm.querySelector("#genre").value;
